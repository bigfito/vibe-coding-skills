...
// bin/agent-skills.cjs para que la comprobación previa y la instalación no
...
function parseArgs(args) {
  ...
  if (!o.ambitos && process.env.AGENT_SKILLS_AMBITO) {
    const pedidos = process.env.AGENT_SKILLS_AMBITO.split(/[\s,]+/)
      .map((x) => (x === "local" || x === "project" ? "proyecto" : x))
      .filter((x) => x === "global" || x === "proyecto");
