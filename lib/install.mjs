...
const ENTORNOS = {
  claude: {
    ...
    baseProyecto: () => join(DESTINO, ".claude"),
  ...
  cursor: {
    ...
    baseProyecto: () => join(DESTINO, ".cursor"),
    pistas: () => [
      join(CASA, ".cursor"),
  ...
  junie: {
    ...
    baseProyecto: () => join(DESTINO, ".junie"),
    pistas: () => [
      join(CASA, ".junie"),
  ...
  antigravity: {
    ...
    baseProyecto: () => join(DESTINO, ".agents"),
    pistas: () => [
      join(CASA, ".gemini"),
...
function carpetas(env, ambito) {
  ...
  const base = ambito === "global" ? e.baseGlobal() : e.baseProyecto();
...
function estaInstalada(env) {
  return ENTORNOS[env].pistas().some((p) => p && existsSync(p));
}
...
function ayuda() {
  console.log(`
${c.bold("agent-skills")} — instala la suite de skills para tu asistente de IA
...
function rutaLegible(ruta) {
  const casa = CASA;
  if (ruta.startsWith(DESTINO + "/") || ruta.startsWith(DESTINO + "\\")) return relative(DESTINO, ruta);
...
const MARCA_INICIO = "<!-- agent-skills: inicio (bloque generado, no editar) -->";
const MARCA_FIN = "<!-- agent-skills: fin -->";
...
async function escribirIndice(env, skills, ctx) {
  ...
  if (inicio !== -1 && fin !== -1 && fin > inicio) {
    nuevo = contenido.slice(0, inicio) + bloque + contenido.slice(fin + MARCA_FIN.length);
  } else {
...
function informarDeteccion(envs) {
  ...
  for (const e of envs) {
    ...
    console.log(c.dim(`    proyecto: ${rutaLegible(v.baseProyecto())}`));
  }
}
